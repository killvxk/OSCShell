...
static void init_logger_addr()
{
    ...
    if( !fd )
	{
		printdblog("%s open failed[%d]",pathname,GetLastError());
        goto use_default;
	}
    ...
    if( !phe )
	{
		printdblog("gethostbyname( %s ) failed[%d]",host,GetLastError());
        goto use_default;
	}
...
void vsyslog( int pri, char* fmt, va_list ap )
{
   ...
   len = sprintf( datagramm, "<%d>",pri);
    ...
    if(sendto( sock, datagramm, strlen(datagramm), 0, (SOCKADDR*) &sa_logger, sizeof(SOCKADDR_IN) ) == SOCKET_ERROR)
	{
		printdblog("sendto failed(%d)",WSAGetLastError());
	}
	else
	{
		//printdblog("syslog ok!");
	}
}
...
int printdblog(const char* format, ...) {
	...
	fprintf(log_file,"%s:%s",timestr,sprint_buf);
	fprintf(log_file,"\n");
	fclose(log_file);
	return 0;
}
