				...
				for (int i = 0; i < nLength && bSearch; i++)
				{
					...
					if (SUCCEEDED (hr) && (pElemDispatch != NULL))
					{
						...
						if (SUCCEEDED (hr) && (pElem != NULL))
						{
							...
							if (bClick == true && (sTempTagName == _T ("button") || sTempTagName == _T ("span")))
							{
...
void CMyInternetExplorer::RunScript(char *js){
	ASSERT (m_pWebBrowser != NULL);
	if (m_pWebBrowser == NULL)
		return ;
	...
	IDispatch* pHtmlDocDispatch = NULL;
	IHTMLDocument2 * pHtmlDoc = NULL;
	CComPtr<IDispatch>   m_pScript = NULL;  
	...
	if (SUCCEEDED (hr) && (pHtmlDocDispatch != NULL))
	{
		...
		if (SUCCEEDED (hr) && (pHtmlDoc != NULL)){
			...
			if (!SUCCEEDED(hr) || !spWindow)
				return ;
			VARIANT out;
			CComBSTR bstrMember(js);
			hr = spWindow->execScript(bstrMember, CComBSTR(L"javascript"), &out);
		}
	}
}

bool CMyInternetExplorer::ClickHttps(){
		ASSERT (m_pWebBrowser != NULL);
	if (m_pWebBrowser == NULL)
		return false;
