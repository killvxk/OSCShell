...
void DATASAFE::DES(uchar *key,uchar *text,uchar *mtext)
{
	uchar tmp[64];
	expand0(key,tmp);
	setkeystar(tmp);
...
void DATASAFE::_DES(uchar *key,uchar *text,uchar *mtext)
{
	uchar tmp[64];
	expand0(key,tmp);
	setkeystar(tmp);
...
void DATASAFE::encrypt0(uchar *text,uchar *mtext)
{
...
void DATASAFE::discrypt0(uchar *mtext,uchar *text)
{
	uchar ll[64],rr[64],LL[64],RR[64];
	uchar tmp[64];
	int i,j;
...
void DATASAFE::expand0(uchar *in,uchar *out)         //取得64位bit=1的顺序号
{
	int divide;
	int i,j;
...
void DATASAFE::compress0(uchar *out,uchar *in)
{
	int times;
	int i,j;
...
void DATASAFE::compress016(uchar *out,uchar *in)
{
	int times;
	int i,j;
...
void DATASAFE::setkeystar(uchar bits[])
{
	int i,j;
...
void DATASAFE::LS(uchar *bits,uchar *buffer,int count)
{
	int i,j;
	for (i=0;i<28;i++)
	{
...
void DATASAFE::son(uchar *cc,uchar *dd,uchar *kk)
{
	int i,j;
	uchar buffer[56];
...
void DATASAFE::ip(uchar *text,uchar *ll,uchar *rr)
{
	int i,j;
	uchar buffer[64];
	uchar tmp[64];
...
void DATASAFE::_ip(uchar *text,uchar *ll,uchar *rr)
{
	int i,j;
	uchar tmp[64];
	for (i=0;i<32;i++)
...
void DATASAFE::F(int n,uchar *ll,uchar *rr,uchar *LL,uchar *RR)
{
	int i,j;
...
void DATASAFE::s_box(uchar *aa,uchar *bb)
//uchar *aa,*bb;
{
	int i,j,k,m;
	int y,z;
...
return:	0 --- SUCC
	...
	bcd
----------------------------------------*/
int DATASAFE::asctobcd(char *bcd, char *asc, int len)
{
char	*p;
int	i, j;
...
return:	asc
---------------------------------------------*/
int DATASAFE::bcdtoasc(char *asc, char *bcd, int len)
{
int  i, j;
	j = (len + len%2) / 2;
	for(i=0; i<j; i++) {
...
文本长度8或16
*/
void DATASAFE::singleDES(char *key,int key_length,char *in,int in_length,char *out)
{
    char bcdtxt[50];
...
文本长度8或16
*/
void DATASAFE::_singleDES(char *key,int key_length,char *in,int in_length,char *out)
{
    char bcdtxt[50];
    memset(bcdtxt,0,sizeof(bcdtxt));
    if(key_length!=8 && key_length!=16) return;
...
char DATASAFE::IntToHex(int len,bool lowcase)
{
...
void DATASAFE::randomDES(char *key,char *in,int in_length,char *out)
{
    char randomKey[10];
    ...
    memcpy(randomKey,&randomTxt[5],3);      //前3
    randomKey[3]=IntToHex(inlen,false);     //前4
    memcpy(&randomKey[4],&key[4],4);            //后4

    memset(randombcdtxt,0,sizeof(randombcdtxt));
    singleDES(randomKey,8,intxt,strlen(intxt),randombcdtxt);  //随机密码加密

    memset(randomTxt,0,sizeof(randomTxt));
    memcpy(randomTxt,randomKey,4);                              
    memcpy(&randomTxt[4],randombcdtxt,4);                       //密文前4
    singleDES(key,8,randomTxt,8,out);
    strcat(out,&randombcdtxt[4]);                               //后续密文
}
void DATASAFE::_randomDES(char *key,char *in,int in_length,char *out)
{
    char randomKey[10];
    char randombcdtxt[60];
    char randomTxt[60];
...
void DATASAFE::_encrypt(char *in,int in_length,char *out)
{
        int t_len=in_length;
        int r_len=t_len%44;
        int num=t_len/44;
...
void DATASAFE::encrypt(char *in,int in_length,char *out)
{
        int t_len=in_length;
        int r_len=t_len%16;
        int num=t_len/16;
