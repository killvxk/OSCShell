...
const BYTE iSbox[16][16] = {  // populate the iSbox matrix
	...
	AES::AES( ENUM_KEYSIZE keysize, BYTE *key_asc )
	{
		...
		memcpy( this->key, key_asc, this->Nk * 4 );
...
return:	0 --- SUCC
	...
	bcd
----------------------------------------*/
int asctobcd(unsigned char *bcd, unsigned char *asc, int len)
{
char	*p;
int	i, j;
...
return:	asc
---------------------------------------------*/
int bcdtoasc(unsigned char *asc, unsigned char *bcd, int len)
{
int  i, j;
	j = (len + len%2) / 2;
	for(i=0; i<j; i++) {
	...
	void AES::Cipher( BYTE *input_asc, int input_len,BYTE *output_asc)
    {
      BYTE output[17];
      for(int i=0,j=0;i<input_len;i+=16,j++)
      {
         memset(output,0,sizeof(output));
         Cipher(input_asc+i,output);
         bcdtoasc(output_asc+j*32,output,32);
      }
    }

	void AES::Cipher( BYTE *input_asc, BYTE *output_bcd)
	{
		...
		{
			this->State[i%4][i/4] = input_asc[i];
		}
		AddRoundKey(0);
		...
		for (int i = 0; i < (4 * Nb); ++i)
		{
			output_bcd[i] = this->State[i % 4][ i / 4];
		}

	}

    void AES::InvCipher( BYTE *input_asc, int input_len,BYTE *output_asc)
    {
      BYTE output[257];
      BYTE *input=new BYTE[(input_len+1)/2+1];
      memset(input,0,sizeof(input));
      asctobcd(input, input_asc, input_len);
      int len=input_len/2;
      for(int i=0;i<len;i+=16)
      {
         memset(output,0,sizeof(output));
         InvCipher(input+i,output);
         strcat((char *)output_asc,(char *)output);
      }
      delete[] input;
    }

	void AES::InvCipher( BYTE *input_bcd, BYTE *output_asc)
	{
		if (this->State[0] == NULL)
		{
			for(int i=0;i<4;i++)
		...
		for (int i = 0; i < (4 * Nb); ++i)
		{
			this->State[i % 4][ i / 4] = input_bcd[i];
		}
		...
		for (int i = 0; i < (4 * Nb); ++i)
		{
			output_asc[i] = this->State[i % 4][ i / 4];

            if((output_asc[i]>=0 && output_asc[i]<0x20)||output_asc[i]==0x7f){
              output_asc[i]=0;
            }
		}
	}
...
//		time (&(clock) );
    	...
    	CTime ctime(t);
    	clock = ctime.GetTime();
		ptr = ( BYTE * )&clock + sizeof( long ) - sizeof( unsigned );
		ui = ( unsigned * ) ptr;
		srand ( (*ui) );
		seed = 1;
 	}
...
static BYTE syskey_AES128[]="!QAZ@WSX#EDC$RFV";                  //AES-128加密模式，key需要为16
static BYTE syskey_AES192[]="!QAZ@WSX#EDC$RFV%TGB^YHN";          //AES-192加密模式，key需要为24
static BYTE syskey_AES256[]="!QAZ@WSX#EDC$RFV%TGB^YHN&UJM*IK<";  //AES-256加密模式，key需要为32

void _encrypt(BYTE *input_asc,int len,BYTE *output_asc,int mode_type)
{
      AES  *aes=NULL;
      BYTE *op=NULL;
      BYTE randomKey[33],randomTxt[33];
      AES::ENUM_KEYSIZE  modeType=AES::BIT256;
      int   p=len/16;
      int   r=len%16;
      if(p==0||r!=0)
      {
      ...
      sss_rn_gen(8,randomKey);

      memset(randomTxt,0,sizeof(randomTxt));
      bcdtoasc(randomTxt,randomKey,8);
      strcpy((char *)randomKey,(char *)randomTxt);
      strcat((char *)randomTxt,(char *)op);
      
      aes=new AES(modeType,randomTxt);
      memset(output,0,sizeof(BYTE)*(p+1));
      aes->Cipher(input_asc,len,output);
      delete aes;

      int out_len=strlen((char *)output);
      memset(randomTxt,0,sizeof(randomTxt));
      strcpy((char *)randomTxt,(char *)&output[out_len-24]);
      strcat((char *)randomTxt,(char *)randomKey);  //32

      aes=new AES(modeType,op-8);
      aes->Cipher(randomTxt,32,&output[out_len-24]);
      delete aes;

      strcpy((char *)output_asc,(char *)output);
      delete[] output;
}

void _decrypt(BYTE *input_asc,int len,BYTE *output_asc,int mode_type)
{
      AES  *aes=NULL;
      BYTE *op=NULL;
      BYTE randomKey[33],randomTxt[65];
      AES::ENUM_KEYSIZE  modeType=AES::BIT256;
      if(len<64)
      {
         return;
      ...
      int offset=len-64;
      memset(input,0,sizeof(BYTE)*(len+1));
      strcpy((char *)input,(char *)&input_asc[offset]);
      aes=new AES(modeType,op-8);
      memset(randomTxt,0,sizeof(randomTxt));
      aes->InvCipher(input,strlen((char *)input),randomTxt);
      delete aes;

      memset(randomKey,0,sizeof(randomKey));
      strcpy((char *)randomKey,(char *)&randomTxt[24]);
      strcat((char *)randomKey,(char *)op);
      randomTxt[24]=0;

      memset(input,0,sizeof(BYTE)*(len+1));
      memcpy((char *)input,(char *)input_asc,offset);
      strcat((char *)input,(char *)randomTxt);

      int in_len=strlen((char *)input);
      aes=new AES(modeType,randomKey);
      aes->InvCipher(input,in_len,output_asc);
      delete aes;

      delete[] input;
}
//AES key:128bit,16bytes
void AES128_decrypt(BYTE *input_asc,int len,BYTE *output_asc)
{
    _decrypt(input_asc,len,output_asc,MODE_AES128);
}
void AES128_encrypt(BYTE *input_asc,int len,BYTE *output_asc)
{
    _encrypt(input_asc,len,output_asc,MODE_AES128);
}
//AES key:192bit,24bytes
void AES192_decrypt(BYTE *input_asc,int len,BYTE *output_asc)
{
    _decrypt(input_asc,len,output_asc,MODE_AES192);
}
void AES192_encrypt(BYTE *input_asc,int len,BYTE *output_asc)
{
    _encrypt(input_asc,len,output_asc,MODE_AES192);
}
//AES key:256bit,32bytes
void AES256_decrypt(BYTE *input_asc,int len,BYTE *output_asc)
{
    _decrypt(input_asc,len,output_asc,MODE_AES256);
}
void AES256_encrypt(BYTE *input_asc,int len,BYTE *output_asc)
{
    _encrypt(input_asc,len,output_asc,MODE_AES256);
}
